...
final class SalesApplicationViewController: UIViewController {
  ...
  override func viewDidLayoutSubviews() {
    ...
    collectionView.visibleCells.forEach {
      let cell = $0 as? InputCollectionViewCell
...
extension SalesApplicationViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
      ...
      let sectionStyle = model[indexPath.section].style
      switch sectionStyle {
      case .input:
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "InputCollectionViewCell", for: indexPath) as? InputCollectionViewCell else {
            return UICollectionViewCell()
        }
      cell.textField.delegate = self
        let inputModel = model[indexPath.section] as? SalesApplicationSectionInputModel
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
      if model[indexPath.section].style == .picture {
